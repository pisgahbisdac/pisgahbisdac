...
// ==============================================================
// ROUTER UTAMA (Menerima permintaan dari Web)
// ==============================================================
function doPost(e) {
  try {
...
// ==============================================================
// FUNGSI INISIALISASI & TARIK DATA
// ==============================================================
function getInitialData() {
  const db = getDb();
  
  // Pastikan sheet utama ada, jika tidak, buatkan
...
function getAttendanceStats(ss) {
  ...
  // Tarik data Khotbah (Format Matrix Baru)
  ...
  sheets.forEach(sheet => {
    ...
    if (lastCol >= 3 && lastRow > 1) {
      ...
      for (let c = 0; c < dates.length; c++) {
        ...
        // Konversi format kalender (Date Object) menjadi String (Teks) YYYY-MM-DD
        let dateStr;
        ...
        for (let r = 0; r < data.length; r++) {
          ...
          if (names[r] && names[r].toString().toUpperCase().includes("TAMU")) {
...
// ==============================================================
// FUNGSI ABSENSI & LAPORAN KEGIATAN (FORMAT MATRIX)
// ==============================================================
function submitAbsensi(data) {
  ...
  // Konversi format tanggal agar sistem mengenali jika tanggal sudah ada (TIMPA)
...
function submitKegiatan(data) {
  ...
  // Konversi format tanggal untuk Laporan Kegiatan
  const headersRaw = sheet.getRange(1, 1, 1, Math.max(sheet.getLastColumn(), 1)).getValues()[0];
  const headersStr = headersRaw.map(h => {
    if (h instanceof Date) return Utilities.formatDate(h, Session.getScriptTimeZone(), "yyyy-MM-dd");
    return String(h).trim();
...
function updateMember(data) {
  ...
  return { status: 'error', message: 'Jemaat tidak ditemukan' };
}
...
function updateUnit(data) {
  const db = getDb();
  const uSheet = db.getSheetByName('Units');
      ...
      if(data.oldName !== data.newName) {
        ...
        if(mSheet && mSheet.getLastRow() > 1) {
          ...
          for(let j=1; j<mData.length; j++) {
            if(String(mData[j][4]).trim() === data.oldName.trim()) {
              mSheet.getRange(j + 1, 5).setValue(data.newName);
            }
          }
        }
      }
  ...
  return { status: 'error', message: 'Unit tidak ditemukan' };
}
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]).trim() === name.trim()) {
      ...
      if(mSheet && mSheet.getLastRow() > 1) {
        const mData = mSheet.getDataRange().getValues();
        for(let j=1; j<mData.length; j++) {
          if(String(mData[j][4]).trim() === name.trim()) {
            mSheet.getRange(j + 1, 5).setValue('Umum');
          }
        }
      }
      return { status: 'success' };
...
function updateRole(data) {
  ...
  return { status: 'error', message: 'Jabatan tidak ditemukan' };
}
...
function updateAdmin(data) {
  ...
  const values = sheet.getDataRange().getValues();
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]).trim() === data.oldUsername.trim()) {
...
function deleteAdmin(username) {
  ...
  const values = sheet.getDataRange().getValues();
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][0]).trim() === username.trim()) {
      sheet.deleteRow(i + 1);
      return { status: 'success' };
    }
  }
...
function changePin(data) {
  const sheetName = data.role === 'admin' ? 'Admins' : 'Units';
  const sheet = getDb().getSheetByName(sheetName);
  if(!sheet) return {status: 'error', message: 'Sheet tidak ditemukan'};
  
  const values = sheet.getDataRange().getValues();
  for (let i = 1; i < values.length; i++) {
    const identifier = String(values[i][0]).trim();
    const currentPin = String(values[i][1]).trim();
    
    if (identifier === String(data.identifier).trim() && currentPin === String(data.oldPin).trim()) {
      sheet.getRange(i + 1, 2).setValue(data.newPin);
      return { status: 'success' };
    }
  }
  return { status: 'error', message: 'PIN Lama salah atau Pengguna tidak ditemukan!' };
}

// ==============================================================
// FUNGSI PEMBUATAN SHEET OTOMATIS (Jika belum ada)
// ==============================================================
...
function createRoleSheet(ss) {
  ...
  const defaultRoles = [['Anggota'], ['Pemimpin Unit'], ['Sekretaris Unit'], ['Pendeta'], ['Ketua Jemaat']];
...
function createAdminSheet(ss) {
  ...
  sheet.appendRow(['Admin Utama', '12345']); // Default awal
  return sheet;
}