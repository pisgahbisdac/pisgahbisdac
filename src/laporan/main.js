    ...
    `;
      ...
      if (recent.length === 0) {
        ...
        let desktopHtml = recent.map(x => {
          ...
          return `<tr><td class="fit-col">${fmtDate(x.date)}</td><td class="fit-col"><span class="badge ${x.badge}">${x.label}</span></td><td style="max-width:250px; white-space:normal; word-wrap:break-word;"><span class="badge ${getCatBadge(x.income_type || x.department, x.kind !== 'income')}">${x.income_type || x.department}</span><br><div style="font-size:12px; color:var(--text-muted, var(--text4)); margin-top:4px;">${shouldHide ? '***' : (x.note || '-')}</div></td><td class="fit-col"><div style="display:flex; align-items:center; gap:6px;"><span class="badge badge-gray" title="${x.receipt_no}" style="max-width:100px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block;vertical-align:middle;">${shortReceipt}</span><span style="display:inline-flex;align-items:center;vertical-align:middle;">${photoBtn}</span></div></td><td class="fit-col ${x.style}" style="text-align:right">${x.sign}${fmt(x.amount)}</td></tr>`;
        }).join('');
