import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */const A="https://script.google.com/macros/s/AKfycbxh6l6elvmca6j6snhZAH-YtCDtExU_UPcFm5e3_T-JDsIriixxRY2JYvcZvfRVASeX/exec";function E(){const e=localStorage.getItem("BISDAC_api_url");return e&&e.trim()!==""?e.trim():A}let u=null,c=[];function m(e){return new Intl.NumberFormat("id-ID").format(e)}function b(e){if(!e)return"-";const t=new Date(e);if(isNaN(t))return e;const n=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];return`${t.getDate()} ${n[t.getMonth()]} ${t.getFullYear()}`}async function w(e,t={}){const n=new URL(E());n.searchParams.set("action",e),n.searchParams.set("token",localStorage.getItem("BISDAC_token")||""),n.searchParams.set("_t",Date.now());for(let l in t)n.searchParams.set(l,t[l]);const a=await(await fetch(n.toString(),{method:"GET",redirect:"follow"})).json();if(!a.success)throw new Error(a.message||"API Error");return a}async function x(e,t){const n={action:e,token:localStorage.getItem("BISDAC_token")||"",data:t},a=await(await fetch(E(),{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},redirect:"follow",body:JSON.stringify(n)})).json();if(!a.success)throw new Error(a.message||"API Error");return a}function I(){const e=localStorage.getItem("BISDAC_token"),t=localStorage.getItem("BISDAC_role"),n=localStorage.getItem("BISDAC_name"),o=["Admin","Bendahara","Ketua Jemaat","Pendeta"].includes(t);e&&o?(u={token:e,role:t,name:n},document.getElementById("loginBtn").style.display="none",document.getElementById("adminControls").style.display="flex",document.getElementById("userNameDisplay").textContent=`Hi, ${n}`,document.querySelectorAll(".admin-only-field").forEach(a=>a.style.display="block")):(u=null,document.getElementById("loginBtn").style.display="block",document.getElementById("adminControls").style.display="none",document.querySelectorAll(".admin-only-field").forEach(a=>a.style.display="none"))}function v(e){const t=document.getElementById("inventoryGrid");if(!e||e.length===0){t.innerHTML='<div style="grid-column: 1 / -1; text-align:center; padding: 40px; color: rgba(255,255,255,0.6);">Belum ada data inventaris.</div>';return}t.innerHTML=e.map(n=>{const o=n.photo?n.photo:"https://images.unsplash.com/photo-1548625361-ec8587d60f58?w=500&q=80";return`
    ...
    `),s.document.close()},document.getElementById("editBtn").onclick=()=>{document.getElementById("detailModal").style.display="none",k(t)},document.getElementById("deleteBtn").onclick=async()=>{if(!confirm("Hapus aset ini?"))return;const i=document.getElementById("deleteBtn"),r=i.innerHTML;i.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Hapus...',i.disabled=!0;try{await x("deleteInventory",{id:t.id}),showCustomAlert("Berhasil dihapus!","success"),document.getElementById("detailModal").style.display="none",p()}catch(d){showCustomAlert(d.message,"error")}finally{i.innerHTML=r,i.disabled=!1}}};window.closeFormModal=function(){document.getElementById("formModal").style.display="none"};function k(e=null){document.getElementById("formModal").style.display="flex";const t=document.getElementById("photoPreview");t.style.display="none",document.getElementById("formPhoto").value="",window.currentPhotoBase64="",e?(document.getElementById("formTitle").textContent="Edit Aset",document.getElementById("formId").value=e.id,document.getElementById("formName").value=e.name,document.getElementById("formDate").value=e.date_acquired?e.date_acquired.substring(0,10):"",document.getElementById("formValue").value=e.value?m(e.value):"",document.getElementById("formLocation").value=e.location,document.getElementById("formCategory").value=e.category||"",document.getElementById("formSource").value=e.source||"",document.getElementById("formTaksasi").value=e.taksasi?m(e.taksasi):"",document.getElementById("formPic").value=e.pic,document.getElementById("formQty").value=e.qty||1,document.getElementById("formUnit").value=e.unit||"Unit",document.getElementById("formSubItems").value=e.sub_items||"",e.photo&&(document.getElementById("photoPreviewImg").src=e.photo,t.style.display="block")):(document.getElementById("formTitle").textContent="Tambah Aset Baru",document.getElementById("formId").value="",document.getElementById("formName").value="",document.getElementById("formDate").value="",document.getElementById("formValue").value="",document.getElementById("formLocation").value="",document.getElementById("formCategory").value="",document.getElementById("formSource").value="",document.getElementById("formTaksasi").value="",document.getElementById("formPic").value="",document.getElementById("formQty").value="1",document.getElementById("formUnit").value="Buah",document.getElementById("formSubItems").value="")}document.getElementById("formPhoto").addEventListener("change",function(e){const t=e.target.files[0];if(!t)return;const n=new FileReader;n.onload=function(o){const a=new Image;a.onload=function(){const l=document.createElement("canvas"),i=800,r=800;let d=a.width,s=a.height;d>s?d>i&&(s*=i/d,d=i):s>r&&(d*=r/s,s=r),l.width=d,l.height=s,l.getContext("2d").drawImage(a,0,0,d,s);const g=l.toDataURL("image/jpeg",.6);window.currentPhotoBase64=g,document.getElementById("photoPreviewImg").src=g,document.getElementById("photoPreview").style.display="block"},a.src=o.target.result},n.readAsDataURL(t)});window.showCustomAlert=function(e,t="success"){const n=document.getElementById("customAlertModal"),o=document.getElementById("alertTitle"),a=document.getElementById("alertMessage"),l=document.getElementById("alertIcon");a.textContent=e,t==="error"?(o.textContent="Gagal",o.style.color="#ef4444",l.innerHTML='<i class="fa-solid fa-circle-exclamation"></i>',l.style.color="#ef4444"):t==="warning"?(o.textContent="Perhatian",o.style.color="#d4af37",l.innerHTML='<i class="fa-solid fa-triangle-exclamation"></i>',l.style.color="#d4af37"):(o.textContent="Berhasil",o.style.color="var(--accent)",l.innerHTML='<i class="fa-solid fa-circle-check"></i>',l.style.color="var(--accent)"),n.style.display="flex"};function h(e){let t=e.target.value.replace(/[^0-9]/g,"");t?e.target.value=new Intl.NumberFormat("id-ID").format(t):e.target.value=""}document.addEventListener("DOMContentLoaded",()=>{I(),p(),document.getElementById("formValue").addEventListener("input",h),document.getElementById("formTaksasi").addEventListener("input",h),document.getElementById("loginBtn").addEventListener("click",()=>{document.getElementById("loginModal").style.display="flex"}),document.getElementById("doLoginBtn").addEventListener("click",async()=>{const e=document.getElementById("loginUsername").value,t=document.getElementById("loginPassword").value;if(!e||!t)return showCustomAlert("Isi username dan password","error");const n=document.getElementById("doLoginBtn");n.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Loading...',n.disabled=!0;try{const o=await w("login",{username:e,password:t});if(o.token){localStorage.setItem("BISDAC_token",o.token);const a=o.role||o.user&&o.user.role||"",l=o.nama||o.user&&o.user.nama||o.user&&o.user.name||"";localStorage.setItem("BISDAC_role",a),localStorage.setItem("BISDAC_name",l)}}catch(o){showCustomAlert(o.message,"error")}finally{n.innerHTML="Masuk",n.disabled=!1,document.getElementById("loginModal").style.display="none",I(),u&&p()}}),document.getElementById("logoutBtn").addEventListener("click",()=>{document.getElementById("logoutModal").style.display="flex"}),document.getElementById("doLogoutBtn").addEventListener("click",()=>{localStorage.removeItem("BISDAC_token"),localStorage.removeItem("BISDAC_role"),localStorage.removeItem("BISDAC_name"),I(),v(c),document.getElementById("logoutModal").style.display="none"}),document.getElementById("addBtn").addEventListener("click",()=>{k()}),document.getElementById("saveBtn").addEventListener("click",async()=>{const e=document.getElementById("formName").value,t=document.getElementById("formDate").value,n=document.getElementById("formValue").value.replace(/\./g,""),o=document.getElementById("formLocation").value,a=document.getElementById("formCategory").value,l=document.getElementById("formSource").value,i=document.getElementById("formTaksasi").value.replace(/\./g,""),r=document.getElementById("formPic").value,d=document.getElementById("formId").value,s=document.getElementById("formQty").value,f=document.getElementById("formUnit").value,g=document.getElementById("formSubItems").value;if(!e||!o||!r||!a||!l||!s||!f)return showCustomAlert("Mohon lengkapi field wajib (*)","error");const B={isUpdate:!!d,id:d,name:e,date_acquired:t,value:n,location:o,category:a,source:l,taksasi:i,pic:r,qty:s,unit:f,sub_items:g};window.currentPhotoBase64&&(B.photo_base64=window.currentPhotoBase64);const y=document.getElementById("saveBtn");y.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i> Menyimpan...',y.disabled=!0;try{await x("saveInventory",B),showCustomAlert("Berhasil disimpan!","success"),closeFormModal(),p()}catch(C){showCustomAlert(C.message,"error")}finally{y.innerHTML="Simpan Data",y.disabled=!1}}),document.getElementById("searchInput").addEventListener("input",e=>{const t=e.target.value.toLowerCase(),n=c.filter(o=>o.name.toLowerCase().includes(t)||o.location.toLowerCase().includes(t)||o.pic.toLowerCase().includes(t));v(n)})});
