import"./modulepreload-polyfill-B5Qt9EMX.js";/* empty css              */const se="https://script.google.com/macros/s/AKfycbxh6l6elvmca6j6snhZAH-YtCDtExU_UPcFm5e3_T-JDsIriixxRY2JYvcZvfRVASeX/exec";function Y(){const e=localStorage.getItem("BISDAC_api_url");return e&&e.trim()!==""?e.trim():se}let z=null,k=[];function L(e){return new Intl.NumberFormat("id-ID").format(e)}function X(e){if(!e)return"-";const o=new Date(e);if(isNaN(o))return e;const i=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];return`${o.getDate()} ${i[o.getMonth()]} ${o.getFullYear()}`}async function Z(e,o={}){const i=new URL(Y());i.searchParams.set("action",e),i.searchParams.set("token",localStorage.getItem("BISDAC_token")||""),i.searchParams.set("_t",Date.now());for(let n in o)i.searchParams.set(n,o[n]);const s=await(await fetch(i.toString(),{method:"GET",redirect:"follow"})).json();if(!s.success)throw new Error(s.message||"API Error");return s}async function ee(e,o){const i={action:e,token:localStorage.getItem("BISDAC_token")||"",data:o},s=await(await fetch(Y(),{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},redirect:"follow",body:JSON.stringify(i)})).json();if(!s.success)throw new Error(s.message||"API Error");return s}function N(e,o){return e?e.split(",").map(t=>t.trim()).includes(o):!1}function W(){const e=localStorage.getItem("BISDAC_token"),o=localStorage.getItem("BISDAC_role"),i=localStorage.getItem("BISDAC_name"),t=o&&(N(o,"Admin")||N(o,"Bendahara")||N(o,"Diakon")||N(o,"Ketua Jemaat")||N(o,"Pendeta"));e&&t?(z={token:e,role:o,name:i},document.getElementById("loginBtn").style.display="none",document.getElementById("adminControls").style.display="flex",document.getElementById("userNameDisplay").textContent=`Hi, ${i}`,document.querySelectorAll(".admin-only-field").forEach(s=>s.style.display=s.dataset.display||"block")):(z=null,document.getElementById("loginBtn").style.display="block",document.getElementById("adminControls").style.display="none",document.querySelectorAll(".admin-only-field").forEach(s=>s.style.display="none"))}window.currentViewMode=localStorage.getItem("BISDAC_invViewMode")||"grid";window.changeViewMode=function(e){window.currentViewMode=e,localStorage.setItem("BISDAC_invViewMode",e),e==="grid"?(document.getElementById("btnViewGrid").classList.add("active"),document.getElementById("btnViewList").classList.remove("active")):(document.getElementById("btnViewList").classList.add("active"),document.getElementById("btnViewGrid").classList.remove("active"));const o=document.getElementById("searchInput").value.toLowerCase();if(o){const i=k.filter(t=>t.name.toLowerCase().includes(o)||t.location.toLowerCase().includes(o)||t.pic.toLowerCase().includes(o));F(i)}else F(k)};function F(e){le();const o=document.getElementById("inventoryGrid");if(!e||e.length===0){o.innerHTML='<div style="grid-column: 1 / -1; text-align:center; padding: 40px; color: rgba(255,255,255,0.6);">Belum ada data inventaris.</div>';return}const i=window.currentViewMode==="list";o.className=i?"inventory-list":"inventory-grid",o.innerHTML=e.map(t=>{const s=t.photo?t.photo:"https://images.unsplash.com/photo-1548625361-ec8587d60f58?w=500&q=80",n=t.status==="Disposed",a=n?"opacity: 0.7; filter: grayscale(80%); border: 1px solid rgba(239, 68, 68, 0.3);":"",m=n?'<div class="inv-badge-status" style="background:rgba(239, 68, 68, 0.9); color:white; font-weight:bold;"><i class="fa-solid fa-ban"></i> DISPOSED</div>':`<div class="inv-badge-status">${t.category||"Uncategorized"}</div>`,g=z?`<input type="checkbox" class="bulk-qr-checkbox" value="${t.id}" onclick="event.stopPropagation(); window.toggleBulkPrintButton();" style="position:absolute; top:15px; left:15px; z-index:20; width:20px; height:20px; cursor:pointer;" title="Pilih untuk cetak QR">`:"";return i?`
          ...
          <div class="inv-list-info">
            <div style="flex:1;">
              ...
              <div style="display: flex; flex-wrap:wrap; gap: 16px; margin-bottom: 5px; margin-top:12px;">
                <div class="inv-asset-meta"><i class="fa-solid fa-barcode"></i> ${t.id}</div>
                <div class="inv-asset-meta"><i class="fa-regular fa-user"></i> ${t.pic}</div>
                <div class="inv-asset-meta"><i class="fa-solid fa-location-dot"></i> ${t.location||"-"}</div>
              </div>
            </div>
            ...
            ${z?`
              <div style="min-width: 150px; text-align:right; border-left:1px solid var(--border-light); padding-left:24px;">
                ...
                <div class="inv-asset-value" style="margin-top:4px;">Rp ${L(t.taksasi||t.value||0)}</div>
        ...
        <div class="inv-asset-card" style="${a}" onclick="window.viewDetail('${t.id}')">
          ...
          <div class="inv-asset-info">
            ...
            <div style="display: flex; flex-direction: column; gap: 6px; margin-bottom: 20px; margin-top: 8px;">
              <div class="inv-asset-meta"><i class="fa-solid fa-barcode" style="width:16px;"></i> ${t.id}</div>
              <div class="inv-asset-meta"><i class="fa-regular fa-user" style="width:16px;"></i> ${t.pic}</div>
              <div class="inv-asset-meta"><i class="fa-solid fa-location-dot" style="width:16px;"></i> ${t.location||"-"}</div>
            </div>
            ...
            ${z?`
              <div style="display:flex; justify-content:space-between; align-items:flex-end; border-top:1px solid var(--border-light); padding-top:16px; margin-top:auto;">
                <div>
                  <div style="font-size:0.75rem; color:var(--text-muted); font-weight:700; text-transform:uppercase; letter-spacing:0.5px;">Nilai Aset</div>
                  <div class="inv-asset-value">Rp ${L(t.taksasi||t.value||0)}</div>
                </div>
              </div>
            `:""}
          </div>
      ...
      `}).join("")}async function j(){try{const e=document.getElementById("inventoryGrid");e.innerHTML='<div style="grid-column: 1 / -1; text-align:center; padding: 60px;"><i class="fa-solid fa-spinner fa-spin fa-2x" style="color:var(--accent-dark);"></i><p style="margin-top:15px; color:var(--text-muted); font-weight: 500;">Memuat koleksi inventaris...</p></div>',k=(await Z("getInventory")).data||[],k.sort((s,n)=>new Date(n.created_at)-new Date(s.created_at)),F(k);const t=new URLSearchParams(window.location.search).get("id");t&&setTimeout(()=>{window.viewDetail(t)},300)}catch(e){document.getElementById("inventoryGrid").innerHTML=`<div style="grid-column: 1 / -1; text-align:center; padding: 40px; color: #ef4444;">Gagal memuat data: ${e.message}</div>`}}window.viewDetail=function(e){const o=k.find(r=>r.id===e);if(!o)return;document.getElementById("detailName").textContent=o.name,document.getElementById("detailId").textContent=o.id,document.getElementById("detailCategory").textContent=o.category||"Belum Dikategorikan",document.getElementById("detailSource").textContent=o.source||"Tidak Diketahui Asalnya",document.getElementById("detailDate").textContent=X(o.date_acquired),document.getElementById("detailLocation").textContent=o.location,document.getElementById("detailPic").textContent=o.pic,document.getElementById("detailQty").textContent=o.qty||1,document.getElementById("detailUnit").textContent=o.unit||"Unit";const i=document.getElementById("detailStatusContainer"),t=document.getElementById("detailStatusBadge"),s=document.getElementById("detailDisposeInfo");o.status==="Disposed"?(i.style.display="block",t.innerHTML='<i class="fa-solid fa-ban"></i> Disposed',t.style.background="rgba(239, 68, 68, 0.2)",t.style.color="#ef4444",s.style.display="block",document.getElementById("detailDisposeReason").textContent=o.dispose_reason||"-",z&&o.dispose_price?(document.getElementById("detailDisposePrice").textContent=`Rp ${L(o.dispose_price)}`,document.getElementById("detailDisposePrice").parentElement.style.display="block"):document.getElementById("detailDisposePrice").parentElement.style.display="none"):(i.style.display="block",t.innerHTML='<i class="fa-solid fa-circle-check"></i> Active',t.style.background="rgba(74, 222, 128, 0.1)",t.style.color="#4ade80",s.style.display="none");const n=document.getElementById("detailSubItems"),a=document.getElementById("detailSubItemsContainer");o.sub_items?(n.textContent=o.sub_items,a.style.display="flex"):a.style.display="none";const m=document.getElementById("detailPhotosContainer");m.innerHTML="";const g=[o.photo,o.pic2,o.pic3,o.pic4].filter(r=>r);g.length>0?(g.forEach(r=>{const d=document.createElement("img");d.src=r,d.style.height="180px",d.style.maxWidth="100%",d.style.borderRadius="12px",d.style.objectFit="contain",d.style.border="1px solid var(--border-light)",d.style.flexShrink="0",d.style.backgroundColor="#f8f9fa",d.style.cursor="pointer",d.onclick=()=>{document.getElementById("imagePreviewFull").src=r,document.getElementById("imagePreviewModal").style.display="flex"},m.appendChild(d)}),m.style.display="flex"):m.style.display="none",z?(document.getElementById("detailValueContainer").style.display="block",document.getElementById("detailValue").textContent=`Rp ${L(o.value)}`,document.getElementById("detailTaksasiContainer").style.display="block",document.getElementById("detailTaksasi").textContent=`Rp ${L(o.taksasi||0)}`,document.getElementById("detailAdminActions").style.display="flex"):(document.getElementById("detailValueContainer").style.display="none",document.getElementById("detailTaksasiContainer").style.display="none",document.getElementById("detailAdminActions").style.display="none");const u=window.location.origin+window.location.pathname+"?id="+o.id,y=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(u)}`;document.getElementById("qrCodeImg").src=y,document.getElementById("qrCodeId").textContent=o.id,document.getElementById("detailModal").style.display="flex",window.printDirectThermal=async()=>{try{if(!("serial"in navigator))return showCustomAlert("Browser Anda tidak mendukung Direct Print. Gunakan Google Chrome/Edge di PC atau Chrome di Android.","error");const r=document.getElementById("detailName").textContent,d=document.getElementById("detailId").textContent.replace("ID: ",""),b=document.getElementById("qrCodeImg").src,c=document.createElement("canvas");c.width=384,c.height=520;const l=c.getContext("2d"),w=c.width/2;l.fillStyle="#ffffff",l.fillRect(0,0,c.width,c.height);const C=20,x=12,v=12,S=c.width-24,B=c.height-24;l.strokeStyle="#000000",l.lineWidth=5,l.beginPath(),l.moveTo(x+C,v),l.lineTo(x+S-C,v),l.quadraticCurveTo(x+S,v,x+S,v+C),l.lineTo(x+S,v+B-C),l.quadraticCurveTo(x+S,v+B,x+S-C,v+B),l.lineTo(x+C,v+B),l.quadraticCurveTo(x,v+B,x,v+B-C),l.lineTo(x,v+C),l.quadraticCurveTo(x,v,x+C,v),l.closePath(),l.stroke(),l.fillStyle="#000000",l.textAlign="center",l.textBaseline="top",l.font="bold 26px monospace";let U=r.toUpperCase();if(U.length<=24)l.fillText(U,w,45);else{let p=U.lastIndexOf(" ",24);p===-1&&(p=22);let h=U.substring(0,p),$=U.substring(p).trim();$.length>24&&($=$.substring(0,21)+"..."),l.fillText(h,w,30),l.fillText($,w,60)}const f=new Image;f.crossOrigin="Anonymous",await new Promise((p,h)=>{f.onload=p,f.onerror=h,f.src=b});const A=300;l.drawImage(f,w-A/2,95,A,A);const I=new Image;if(I.crossOrigin="Anonymous",await new Promise((p,h)=>{I.onload=p,I.onerror=()=>p(),I.src=window.location.origin+"/icons/PisgahColor.png"}),I.complete&&I.naturalWidth>0){const h=w,$=95+A/2,q=document.createElement("canvas");q.width=80,q.height=80;const _=q.getContext("2d");_.drawImage(I,0,0,80,80);const R=_.getImageData(0,0,80,80),E=R.data;for(let M=0;M<E.length;M+=4)E[M+3]>20&&(E[M]=0,E[M+1]=0,E[M+2]=0,E[M+3]=255);_.putImageData(R,0,0),l.fillStyle="#ffffff",l.beginPath(),l.arc(h,$,80/2+8,0,Math.PI*2),l.fill(),l.drawImage(q,h-80/2,$-80/2,80,80)}l.fillStyle="#000000",l.font="bold 28px monospace",l.fillText(d,w,420),l.strokeStyle="#cccccc",l.lineWidth=1,l.beginPath(),l.moveTo(60,460),l.lineTo(c.width-60,460),l.stroke(),l.fillStyle="#888888",l.font="20px monospace",l.fillText("PISGAH-BISDAC",w,472);const P=document.getElementById("thermalPreviewModal");P.style.cssText="display:flex; position:fixed; top:0; left:0; width:100%; height:100%; z-index:99999; background:#111827; align-items:center; justify-content:center;",document.getElementById("thermalPreviewImg").src=c.toDataURL();const D=document.getElementById("doDirectPrintBtn"),T=D.cloneNode(!0);D.parentNode.replaceChild(T,D),T.onclick=async()=>{try{const p=await navigator.serial.requestPort();await p.open({baudRate:9600});const h=p.writable.getWriter();showCustomAlert("Mencetak...","success");const q=l.getImageData(0,0,c.width,c.height).data,_=Math.ceil(c.width/8),R=c.height,E=new Uint8Array(8+_*R);E[0]=29,E[1]=118,E[2]=48,E[3]=0,E[4]=_&255,E[5]=_>>8&255,E[6]=R&255,E[7]=R>>8&255;let M=8;for(let O=0;O<R;O++)for(let V=0;V<_;V++){let Q=0;for(let H=0;H<8;H++){const K=V*8+H;if(K<c.width){const G=(O*c.width+K)*4,ie=q[G]*.299+q[G+1]*.587+q[G+2]*.114;q[G+3]>128&&ie<128&&(Q|=1<<7-H)}}E[M++]=Q}const oe=new Uint8Array([27,64]),ne=new Uint8Array([27,97,1]),ae=new Uint8Array([10,10,10,10,10,10,10,10,10,10]);await h.write(oe),await h.write(ne),await h.write(E),await h.write(ae),h.releaseLock(),await p.close(),document.getElementById("thermalPreviewModal").style.display="none",showCustomAlert("Berhasil dicetak langsung ke printer thermal!","success")}catch(p){console.error(p),p.name!=="NotFoundError"&&showCustomAlert("Gagal print: "+p.message,"error")}}}catch(r){console.error(r),showCustomAlert("Gagal memuat preview: "+r.message,"error")}},window.printBarcode=()=>{const r=document.getElementById("qrCodeImg").src,d=document.getElementById("detailName").textContent,b=document.getElementById("detailId").textContent.replace("ID: ",""),c=window.open("","_blank");if(!c){showCustomAlert("Pop-up diblokir oleh browser Anda. Izinkan pop-up untuk pisgahbisdac.app agar dapat mencetak.","error");return}c.document.write(`
      <html>
        <head>
  ...
  `;k.forEach(t=>{const s=t.photo?t.photo:"/icons/PisgahLogoColor.png",n=t.status==="Disposed"?`<span style="color:red; font-weight:bold;">Disposed</span><br><span style="font-size:8px;">${t.dispose_reason||""}</span>`:'<span style="color:green; font-weight:bold;">Active</span>';o+=`
        <tr>
          ...
          <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">
            Awal: ${L(t.value||0)}<br>
            <span style="color:#666;">Market Value: ${L(t.taksasi||0)}</span>
          </td>
        </tr>
    `}),o+=`
      </tbody>
